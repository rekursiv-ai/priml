...
Two departures from :class:`~priml.model.attention.SelfAttention`, both
...
from priml.model.attention import layer_window, window_mask
...
from priml.model.rope import rotate_conjugate
    ...
    * **A value gate.** When the caller supplies a value embedding, each head
    ...
    class Config(Fig["ValueGatedAttention"]):
        ...
        def finalize(self) -> Self:
            ...
            if self.window == -1 and self.max_seq_len > 0 and self.depth >= 0:
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        out = self.attention(q, k, v, window=window)
